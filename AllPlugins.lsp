...
(setq physicalPlotterRedeA4 "\\\\arq1\\Canon G3010 Series")                     ; Impressora F�sica A4 em Rede
(setq physicalPlotterServidorA4 "Canon G3010 Series")                           ; Impressora F�sica A4 em Servidor
...
(setvar "BACKGROUNDPLOT" 2)	                                                    ;Aqui eu seto a variável do sistema pra PLOT em foreground e PUBLISH em background (N�mero 2)

;Aqui eu reseto os
(SETQ ORIGPATH (STRCAT (GETENV "ACAD")";"))
(SETQ ONEPATH (STRCAT "I:\\INTERIORES\\1.Padr�o\\Padr�o PB - AutoCad\\Plugins\\PluginsToLoad;I:\\INTERIORES\\1.Padr�o\\Padr�o PB - AutoCad\\Hachuras"));ADD PATHS HERE, NOT TO MANY OR IT GETS CUT OFF
  ...
  (setq nomeescolhido(GetString T "\nDigite um nome para as pranchas:"))
    ...
    (if (setq ss (ssget "_C" p1 p2 '((0 . "INSERT") (2 . "A4-20,A4-25,A4-50,A4-75,A4-100,A4-125,A3-25,A3-50,A3-75,A3-100,A3-125,A2-25,A2-50,A2-75,A2-100,A2-125"))))
        (progn
            ...
            ;Nesta parte, fa�o a l�gica para decidir se a planta � Landscape ou Portrait,
            ;pegando o Bounding Box dela para fazer a matem�tica
...
(if (setq ss2 (ssget "_X" (list '(0 . "INSERT") (cons 2 blocksize))))
	    ...
	    ;(setq plantanumber 1)
			...
			(setq count 0)
      ;Verifica��o da necessidade de imprimir essa planta
...
(defun printsinglesheet()
  ...
  ;L�GICA PARA DECIDIR SE EST� NO SERVIDOR OU NA REDE E ESCOLHER A IMPRESSORA CERTA
...
(defun layout ()
  (setq oldlayer (getvar "CLAYER")) ;Pega a layer atual
	...
	(command "_laythw")               ;Exibe todas as layers para depois apagar as espec�ficas
...
;MOSTRAR LUMINOT�CNICO
(defun luminotecnico ()
  (setq oldlayer (getvar "CLAYER"))
	(command "setvar" "clayer" "0")
	(command "_laythw")
	(command "_.layer" "_freeze" "1 Layout Cotas,1 Layout Texto,2 Hidr�ulica,2 Hidr�ulica Cotas,3 El�trico,3 El�trico Cotas,4 Luminot�cnico Se��es,5 Forro,5 Forro Cotas,6 Piso,6 Piso Cotas,7 Ar Condicionado,7 Ar Condicionado Cotas" "")
  (setvar "CLAYER" oldlayer)
)
